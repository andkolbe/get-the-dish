...
const all = () => Query<IDishes[]>('SELECT dishes.*, users.username FROM dishes JOIN users ON users.id = dishes.userid ORDER BY dishes.created_at DESC');
const one = (id: number) => Query<IDishes[]>('SELECT dishes.*, users.username FROM dishes JOIN users ON users.id = dishes.userid WHERE dishes.id = ?', [id]);
