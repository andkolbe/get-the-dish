...
const updateTokenExpired = (email: string) => Query('UPDATE reset_token SET used = 1, updated_at = CURTIME() WHERE email = ?' , [email]);
const updateTokenSuccessful = (email: string, token: string) => Query('UPDATE reset_token SET used = 2, updated_at = CURTIME() WHERE email = ? AND token = ?' , [email, token]);
